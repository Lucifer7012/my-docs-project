...
<div class="search-shell">
    <section class="search-hero">
        ...
        <p class="search-subtitle">这个页面现在是 Cloudflare 后端驱动的实时搜索页。输入游戏名后，它会尽量聚合出 Google Play、App Store、TapTap、APKPure 等渠道结果，并展示图标、包名和链接。</p>
    ...
    <section class="search-panel">
        ...
        <p class="search-helper">搜索逻辑会先找 Google Play 最佳匹配，再补充其他渠道。图标复制优先复制图片本体，如果浏览器不支持，会自动回退成复制图标链接。</p>
    ...
    <section class="search-note-panel">
        <div class="note-grid">
            ...
            <article class="note-card">
                ...
                <ul>
                    <li><code>GOOGLE_CSE_API_KEY</code></li>
                    <li><code>GOOGLE_CSE_CX</code></li>
                </ul>
            </article>
            <article class="note-card">
                <h3>How It Works</h3>
                <p>Google Play uses Google Custom Search JSON API for discovery. App Store comes from Apple iTunes Search API. TapTap and APKPure are best-effort supplemental matches.</p>
            </article>
        </div>
    </section>
</div>
...
async function runSearch(query) {
    ...
    try {
        ...
        setEmptyState("Search failed. Check your Cloudflare env vars and API configuration.");
    }
}
